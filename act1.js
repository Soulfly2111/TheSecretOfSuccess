...
function icon(canvas,id){const c=canvas.getContext('2d'),r=(x,y,w,h,color)=>{c.fillStyle=color;c.fillRect(x,y,w,h);};if(id==='techKey'){r(3,4,8,8,'#dfb56b');r(5,6,4,4,'#243138');r(10,10,11,3,'#dfb56b');r(17,12,3,5,'#dfb56b');}else if(id==='mats'){r(3,6,18,5,'#9ba6ae');r(3,12,18,5,'#4b6877');r(3,18,18,3,'#293f50');}else{r(4,3,16,19,'#e4d9b8');r(4,3,16,4,id==='serviceBadge'?'#5b9cad':id==='badge'?'#8f5861':'#b4905f');r(6,9,5,6,'#6b8f9a');r(13,9,5,1,'#5a655f');r(13,12,5,1,'#5a655f');r(6,18,12,1,'#5a655f');}}
...
function popup(title,text){$('modal-content').replaceChildren();const h=document.createElement('h2');h.textContent=title;$('modal-content').append(h);for(const line of text){const p=document.createElement('p');p.textContent=line;$('modal-content').append(p);}if(!$('modal').open)$('modal').showModal();}
