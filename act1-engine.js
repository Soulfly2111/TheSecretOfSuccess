...
function act(s,verb,target,item){
 ...
 if(item){
  ...
  if(item==='badge'&&target==='reader'){note('access','Der vorläufige Ausweis öffnet die Personalschleuse.');return 'Piep. Zutritt genehmigt. Auf meinem Ausweis steht „Empfang und Zugangskontrolle“. Eine Schlüsselposition.';}
  ...
  if(item==='serviceBadge'&&target==='technician'){remove(item);note('techAdmitted','Technikerin mit zeitlich begrenztem Ausweis registriert.');return 'Technikerin: „60 Minuten reichen. Jetzt brauche ich freie Zufahrt für meinen Gerätewagen und den Schlüssel zum Technikflur.“';}
  ...
  if(item==='techKey'&&target==='techDoor'){
   ...
   remove(item);note('technicalOpen','Schlüssel K-17 dokumentiert; Technikflur gemeinsam aufgeschlossen.');return 'K-17 passt. Ich begleite die Technikerin hinein. Die Reparatur übernimmt sie.';
  }
  if(item==='badge'&&target==='techDoor')return 'Der Ausweis öffnet die Schleuse, aber keine mechanischen Technikschlösser.';
  return 'Das passt hier nicht. Vielleicht hilft ein Blick in die Unterlagen.';
 }
 if(verb==='Schau an'){
  if(target==='invitation'){note('invitationRead','Personalnummer 4711 · Ansprechpartnerin Frau Seidel, Durchwahl 100. Personaleingang hinten.');return 'EINLADUNG: „Melden Sie sich Montag zum Dienstantritt.“ Personalnummer: 4711. Ansprechpartnerin: Frau Seidel, 100. Kleingedruckt: „Personaleingang hinten.“';}
  ...
  if(target==='reader')return f.access?'Die Schleuse ist freigeschaltet.':'Ohne Ausweis kein Zutritt. Der Ausweis wird leider auf der anderen Seite ausgestellt.';
  ...
  return ({fountain:'Marmor, Wasser und ein sehr teures Echo.',portrait:'Der Onkel. Sogar sein gemaltes Lächeln wirkt wie eine Dienstanweisung.',mats:'Passende Schutzmatten liegen beim Lieferfahrer.',sideDoor:'Eine klemmende Seitentür. Hier beginnt die Karriere hinter den Kulissen.',locker:'Ein schlecht sitzender Pförtnerblazer. Die Ärmel passen eher zu meinen Zukunftsplänen als zu meinen Armen.',badge:'Vorläufiger Mitarbeiterausweis · Nr. 4711 · Empfang und Zugangskontrolle.'})[target]||'Hier lohnt sich ein Gespräch oder ein Blick in die Unterlagen.';
 }
 if(target==='sideDoor'&&['Öffne','Drücke','Ziehe'].includes(verb)){note('sideOpen','Die klemmende Seitentür ist offen. Walter wartet in der Loge.');return 'Mit einem kräftigen Druck springt die Seitentür auf. Walter: „Du bist der Neue für die Pforte?“ Ich: „Da liegt ein Missverständnis vor.“ Walter: „Dann bist du hier richtig.“';}
 ...
 if(verb==='Wähle'){
  ...
  if(target==='keyWrong')return 'Die alte Raumnummer passt nicht zur Kühlanlage. Der Schlüssel bleibt am Brett. Vergleiche den Aushang in der Personalschleuse.';
  if(target==='keyCorrect'){if(!f.keyLogged)return 'Walter hat es erklärt: Erst die Ausgabe ins Schlüsselbuch eintragen.';if(f.technicalOpen)return 'Der Schlüssel wurde bereits für den Technikflur ausgegeben.';add('techKey');return 'Schlüssel T-03 / K-17. Genau der dokumentierte Schlüssel für die Kühlanlage.';}
...
function canEnter(s,room){if(room==='lodge'&&!s.flags.sideOpen)return 'Die Seitentür klemmt. Erst öffnen oder drücken.';if(room==='corridor'&&!s.flags.technicalOpen)return 'Der Technikflur ist verschlossen. Ich brauche einen dokumentierten Schlüssel und die Technikerin.';return '';}
function hint(s){const f=s.flags,has=id=>s.inventory.includes(id);
 ...
 if(!f.sideOpen)return 'Öffne die Seitentür am Haupteingang, dann gehe zur Pförtnerloge.';
 if(!f.employment)return 'Benutze das Haustelefon in der Loge und rufe Frau Seidel zum Dienstantritt an.';
 ...
 if(!f.scheduleRead)return 'Schau die Terminübersicht in der Pförtnerloge an.';
 if(!f.directoryRead)return 'Lies das Telefonverzeichnis in der Loge.';
 if(!f.visitorConfirmed)return 'Rufe Thomas Berger in der IT an, um die Kopiererwartung zu bestätigen.';
 if(!f.visitorDone)return has('visitorBadge')?'Gib dem Übungsbesucher den Ausweis B.':'Nimm den passenden Besucherausweis vom Ausweisständer in der Loge.';
 ...
 if(!f.maintenanceRead)return 'Gleiche den Serviceauftrag mit dem Wartungsbuch in der Loge ab.';
 if(!f.serviceConfirmed)return 'Rufe über das Haustelefon die Haustechnik an.';
 if(!f.techAdmitted)return has('serviceBadge')?'Gib der Technikerin ihren zeitlich begrenzten Ausweis.':'Nimm den Technikausweis vom Ausweisständer in der Loge.';
 ...
 if(!f.access)return 'Benutze deinen Mitarbeiterausweis mit dem Leser in der Personalschleuse.';
 if(!f.roomMapRead)return 'Lies den Aushang hinter dem Ausweisleser in der Personalschleuse.';
 if(!f.keyLogged)return 'Benutze das Schlüsselbuch in der Loge, um die Ausgabe zu dokumentieren.';
 if(!f.technicalOpen)return has('techKey')?'Benutze Schlüssel K-17 mit der Techniktür in der Personalschleuse.':'Nimm am Schlüsselbrett den Schlüssel für T-03 / Kühlung.';
 if(!f.repaired)return 'Gehe in den Technikflur und rede mit der Technikerin. Sie übernimmt die Reparatur.';
 if(!f.uncleMet)return 'Der Direktor ist am Haupteingang eingetroffen. Rede mit ihm.';
 if(!f.audited)return 'Walter ist zurück in der Loge. Übergib ihm die Schicht.';
 if(!f.ritaReady)return 'Rita wartet am Haupteingang. Rede mit ihr über die Poststelle.';
 return 'Benutze den Lastenaufzug im Technikflur. Rita begleitet dich nach unten.';
}
function visible(s,id){const f=s.flags;return !({visitor:!f.briefed||f.visitorDone,walter:f.visitorDone&&!f.repaired,salesman:!f.visitorDone||f.sellerRejected,technician:!f.visitorDone||s.room==='delivery'&&f.technicalOpen,driver:!f.visitorDone,bust:!f.visitorDone||f.accessClear,deliveryNote:!f.visitorDone,mats:!f.visitorDone||s.inventory.includes('mats')||f.liftProtected,uncle:!f.repaired,rita:!f.audited,mara:!f.visitorDone||f.maraHelped||f.repaired,notice:!f.access,techDoor:!f.access,freight:!f.repaired}[id]);}
