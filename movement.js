(function(root){
'use strict';
// All geometry uses a fixed 960 × 540 world; positions represent feet.
const maps={
...
const grid=12,cache={};
function nodes(room){if(cache[room])return cache[room];const points=[];for(let y=300;y<516;y+=grid)for(let x=36;x<948;x+=grid)if(walkable(room,{x,y}))points.push({x,y});return cache[room]=points;}
